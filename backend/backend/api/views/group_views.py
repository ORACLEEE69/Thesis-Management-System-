...
class GroupViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        # Only set default if frontend didn't send a value
    ...
    def get_queryset(self):
        ...
        if self.action in ['retrieve', 'update', 'partial_update', 'destroy']:
            ...
            print(
                f"Is action in excluded list? {self.action not in ['pending_proposals', 'get_current_user_groups', 'approve', 'reject']}")
    ...
    def approve(self, request, pk=None):
        ...
        try:
            ...
            proposer = group.leader if group.leader else (
                group.members.first() if group.members.exists() else request.user)
    ...
    def assign_panel(self, request, pk=None):
        ...
        if not panel_ids:
            return Response({'error': 'panel_ids is required and must be a non-empty list'},
        ...
        if panel_users.count() != len(panel_ids):
            return Response({'error': 'One or more panel members not found or not a panel user'},
    ...
    def remove_panel(self, request, pk=None):
        ...
        try:
            ...
            return Response({'error': 'Panel member not found or user is not a panel user'},
                            status=status.HTTP_404_NOT_FOUND)
