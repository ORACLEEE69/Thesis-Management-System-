...
def get_user_display_name(user):
    ...
    return user.email or user.username or "Unknown User"
...
class ScheduleViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if group.adviser:
            adviser_email = group.adviser.email

        for panel in group.panels.all():
            panel_emails.append(panel.email)

        print('member emails: ', member_emails)
        print('panel emails: ', panel_emails)
    ...
    def validate_schedule_update(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            print("\n" + "=" * 50)
            print("DEBUG: Current group")
            print("ID:", group.id)
            print("Name:", getattr(group, 'name', 'No name'))
            print("Adviser:", group.adviser)
            print("Panels:", [p.id for p in group.panels.all()])
            print("Members:", [m.id for m in group.members.all()])
            print("=" * 50 + "\n")
            return Response({"debug": "check terminal"}, status=200)
